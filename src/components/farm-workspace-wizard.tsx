...
import type { FarmNode, FarmSummary, WorkspaceActuator } from "@/lib/farm-data";
...
export type FarmWorkspaceWizardProps = {
  mode: "onboarding" | "manage";
  /** Live workspace from the dashboard snapshot (manage mode). */
  manageSnapshot?: {
    farms: FarmSummary[];
    nodes: FarmNode[];
    actuators: WorkspaceActuator[];
  };
  /** Bump when snapshot content changes so the form rehydrates from the server. */
  dataRevision?: string;
  onManageSaved?: () => void;
};
...
const defaultActuatorRow = (): ActuatorRowDraft => ({
  farmIndex: 0,
  name: "",
  actuator_type: "valve",
  field_or_location: "",
...
export function FarmWorkspaceWizard({
  mode,
  manageSnapshot,
  dataRevision,
  onManageSaved,
}: FarmWorkspaceWizardProps) {
  ...
  const [actuators, setActuators] = useState<ActuatorRowDraft[]>([defaultActuatorRow()]);
  ...
  useEffect(() => {
    if (mode !== "manage" || !manageSnapshot) return;
    const { farms: list, nodes: nodeList, actuators: actList } = manageSnapshot;
    if (list.length === 0) {
      setFarmCount(1);
      setFarms([emptyFarm()]);
      setNodes([]);
      setActuators([defaultActuatorRow()]);
      setStep(1);
      return;
    }
    const farmRows = list;
    setFarmCount(farmRows.length);
    setFarms(
      farmRows.map((f) => ({
        name: f.name,
        geoQuery: "",
    ...
    const idToIdx = new Map(farmRows.map((f, i) => [f.id, i]));
    setNodes(
      nodeList.map((n) => ({
        farmIndex: idToIdx.get(n.farmId) ?? 0,
        name: n.name,
        role: n.role,
        notes: n.connectivityNotes ?? "",
      })),
    );
    if (actList.length === 0) {
      setActuators([defaultActuatorRow()]);
      ...
      setActuators(
        actList.map((a) => ({
          farmIndex: a.farm_id != null ? (idToIdx.get(a.farm_id) ?? 0) : 0,
          name: a.name,
          actuator_type: a.actuator_type,
          field_or_location: a.field_or_location ?? "",
          notes: a.notes ?? "",
        })),
      );
    }
    setStep(1);
  }, [mode, dataRevision, manageSnapshot]);
  ...
  async function persistWorkspace() {
    ...
    try {
      ...
      const shouldMarkProfile =
        mode === "onboarding" || (mode === "manage" && profile && !profile.onboarding_completed);
      if (shouldMarkProfile) {
      ...
      toast.success(mode === "manage" ? "Farm workspace updated." : "Workspace saved.");
      if (mode === "onboarding") {
        await navigate({ to: "/" });
      } else {
        onManageSaved?.();
  ...
  async function finishOnboarding(skipHardware: boolean) {
    if (!supabase || !user) return;
    if (mode === "manage" && skipHardware) return;
    if (!skipHardware) {
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground text-sm max-w-xl mx-auto">
          {mode === "onboarding"
            ...
            : "Update farm names and weather pins, then register nodes and actuators. Saving replaces your previous farm list and recreates starter plots for each farm."}
        </p>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <Button
              ...
              className="w-full gap-2"
              onClick={() =>
                setActuators((r) => [...r, defaultActuatorRow()])
              }
            >
              <Plus className="h-4 w-4" />
              Add actuator
